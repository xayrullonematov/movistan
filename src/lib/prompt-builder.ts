...
const ENGINEERING_ROOM_PREAMBLE = [
  ...
  ``,
  `## Output budget for artifact suggestions (hard constraints)`,
  ``,
  `These bounds are NOT polite suggestions. The other three agents will`,
  `produce overlapping suggestions in parallel, consensus will deduplicate`,
  `and synthesize, and every excess artifact wastes tokens the room could`,
  `have spent on the next round.`,
  ``,
  `- **At most 4 artifactSuggestions per response.** If your discipline`,
  `  produces more than 4 honest candidates, pick the 4 most load-bearing`,
  `  ones — the ones a peer would have to argue against to disagree with`,
  `  your overall position. Surfacing 7 weak suggestions instead of 3 strong`,
  `  ones lowers signal, not raises it.`,
  `- **Artifact content: ≤ 4 sentences (≤ ~120 words).** Title carries the`,
  `  decision; content carries the reasoning. Multi-paragraph essays here`,
  `  are out of scope — revision and consensus will expand the ones that`,
  `  matter. Be specific and compressible, not exhaustive.`,
  `- **Summary field: ≤ 3 sentences.** Lead with the position; the rest of`,
  `  the response carries the detail.`,
  `- **Risks: only specific, predicate-able failure modes.** "X could fail"`,
  `  is not a risk; "X loses up to N events when Y happens under Z load" is.`,
  `  Vague risks get culled in consensus and don't earn their tokens.`,
  `- **Recommendations: at most 5,** each one sentence. Anything longer`,
  `  belongs in a decision or recommendation artifact, not the recommendations array.`,
  ``,
  `When in doubt, ship fewer, sharper items. The synthesizer's job is to`,
  `merge; your job is to give it precise material to merge.`,
  ``,
